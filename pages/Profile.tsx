...
const ProfilePage: React.FC = () => {
  ...
  const StatCard = ({ icon: Icon, label, value, sublabel, onClick, color }: any) => (
    ...
    >
      ...
      <span className="text-slate-400 text-[10px] font-bold mt-1 uppercase tracking-tight">{sublabel}</span>
    </button>
  );
      ...
      <div className="bg-gradient-to-br from-[#4ea7a1] via-[#86918d] to-[#e68371] rounded-[44px] p-8 text-white mb-8 shadow-2xl shadow-slate-200 relative overflow-hidden">
        <div className="flex items-center gap-6 mb-10 relative z-10">
          ...
          <div className="flex-1">
            ...
            <p className="text-white/80 font-bold text-sm mb-4 uppercase tracking-widest">{user.major}</p>
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-[32px] p-6 relative z-10">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-sm font-black text-white/60 tracking-tight uppercase">Sync Points</span>
          </div>
        </div>
      </div>
      ...
      {showGoalModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white w-full max-w-sm rounded-[44px] p-8 shadow-2xl relative z-10">
            <h3 className="text-2xl font-black text-slate-800 mb-2 tracking-tight">Today's Goal</h3>
            ...
            <div className="relative mb-8">
              <input 
                ...
                className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-5 text-3xl font-black text-slate-800 outline-none focus:border-red-400 focus:bg-white transition-all tracking-tighter"
                autoFocus
              />
            ...
            <div className="flex gap-3">
              <button 
                ...
                className="flex-1 font-black text-slate-400 py-4 rounded-2xl hover:bg-slate-50 transition-colors uppercase text-[10px] tracking-widest"
              >
                Cancel
              </button>
              <button 
                onClick={handleSaveGoal}
                className="flex-1 bg-[#ff6b6b] text-white font-black py-4 rounded-2xl shadow-xl shadow-red-100 hover:bg-red-500 transition-all active:scale-95 uppercase text-[10px] tracking-widest"
              >
                Confirm
              </button>
            </div>
